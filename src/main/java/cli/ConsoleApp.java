...
public class ConsoleApp {
    ...
    private void handleOpen(String[] parts) {
        if (parts.length < 2) {
            System.out.println("Использование: open <code>");
            return;
        }
    ...
    private void handleList() {
        ...
        for (Link l : links) {
            System.out.println(l.code() + " -> " + l.originalUrl()
                    + " | переходы: " + l.clicksDone() + "/" + l.maxClicks()
                    + " | истекает: " + l.expiresAt());
        }
    }

    private void handleDelete(String[] parts) {
        if (parts.length < 2) {
            System.out.println("Использование: delete <code>");
            return;
        }
    ...
    private void handleLimit(String[] parts) {
        if (parts.length < 3) {
            System.out.println("Использование: limit <code> <newLimit>");
            return;
        }
        String code = parts[1];
    ...
    private void handleUser(String[] parts) {
        if (parts.length < 2) {
            System.out.println("Использование:: user <uuid>");
            return;
        }
